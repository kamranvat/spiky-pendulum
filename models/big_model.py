...
class Model(torch.nn.Module):
    def __init__(
        ...
        # bias = False is required for RSTDP optimiser
        self.con1 = torch.nn.Linear(input_size, 100, bias = False)  
        self.lif1 = snntorch.Leaky(0.9, 0.5, surrogate_disable = True)
        self.con2 = torch.nn.Linear(100, 500, bias = False)
        self.lif2 = snntorch.Leaky(0.9, 0.5, surrogate_disable = True)
        self.con3 = torch.nn.Linear(500, 500, bias = False)
        self.lif3 = snntorch.Leaky(0.9, 0.5, surrogate_disable = True)
        self.con4 = torch.nn.Linear(500, output_size, bias = False)
        self.lif4 = snntorch.Leaky(0.9, 0.5, surrogate_disable = True)
        ...
        for step in range(self.spike_time):
            ...
            if use_traces:
                self.optim.update_e_trace(
                    pre_firing=[[x[step], spk1, spk2, spk3]], post_firing=[[spk1, spk2, spk3, spk4]]
                )
