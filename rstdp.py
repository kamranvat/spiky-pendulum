...
class RSTDP(torch.optim.Optimizer):
    def __init__(self, params, time_steps: int, lr: float = 1e-3, **kwargs):
        ...
        defaults = dict(
            ...
            A_plus=kwargs.get("A_plus", 3.0),
            A_minus=kwargs.get("A_minus", 0.0),
        )
        ...
        if time_steps <= 0 or not isinstance(time_steps, (int, np.int_, torch.int)):
            raise ValueError(
                f"Time steps should be a postive integer value. Got {time_steps=}."
            )

        if defaults["lr"] <= 0:
            raise ValueError(
                f'Learning rate should be a positive float value. Got {defaults["lr"]=}.'
            )
        ...
        if defaults["a_plus"] < 0:
            raise ValueError(
                f'a_plus should be a positive value. Got {defaults["a_plus"]=}.'
            )

        if defaults["a_minus"] < 0:
            raise ValueError(
                f'a_minus should be a positive value. Got {defaults["a_minus"]=}.'
            )
        ...
        for group in self.param_groups:
            ...
            for param in group["params"]:
                ...
                group_e.append(
                    torch.zeros((param.shape[0], param.shape[1], self.time_steps + 1))
                )
    ...
    def update_e_trace(self, pre_firing: list, post_firing: list) -> None:
        ...
        for idx, group in enumerate(self.param_groups):
            for i in range(len(group["params"])):
                ...
                dtrace = (
                    -group["pref_a"] * self.pre_trace[idx][i]
                    + group["a_plus"] * pre_firing[idx][i]
                )
                self.pre_trace[idx][i] = self.pre_trace[idx][i] + dtrace

                de_pos = (
                    torch.outer(post_firing[idx][i], self.pre_trace[idx][i])
                    * group["A_plus"]
                )

                # negative STDP update: post before pre
                dtrace = (
                    -group["pref_a"] * self.post_trace[idx][i]
                    + group["a_minus"] * post_firing[idx][i]
                )
                self.post_trace[idx][i] = self.post_trace[idx][i] + dtrace

                de_neg = (
                    torch.outer(self.post_trace[idx][i], pre_firing[idx][i])
                    * group["A_minus"]
                )
                ...
                self.e_trace[idx][i][:, :, self.cur_step] = (
                    -group["pref_e"] * self.e_trace[idx][i][:, :, self.cur_step - 1]
                    + stdp_update
                )
    ...
    def step(self, reward: torch.Tensor, closure=None) -> None:
        ...
        for idx, group in enumerate(self.param_groups):
            for i, p in enumerate(group["params"]):
                ...
                e_trace = (
                    self.e_trace[idx][i][:, :, 1 : self.time_steps]
                    * reward
                    * group["lr"]
                )
